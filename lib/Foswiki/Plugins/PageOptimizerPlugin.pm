...
sub initPlugin {
    ...
    Foswiki::Func::registerRESTHandler(
        ...
        sub {
            ...
            return Foswiki::Plugins::PageOptimizerPlugin::Stats::restStatistics(
                @_);
        }
    );
...
sub completePageHandler {
    ...
    if ( $Foswiki::cfg{PageOptimizerPlugin}{CleanUpHTML} ) {
        ...
        $text =~
          s/<link (.*?rel=["']stylesheet["'].*?)\/>/_processLinkStyle($1)/ge;
    }
    ...
    $text = optimizeJavaScript($text)
      if $Foswiki::cfg{PageOptimizerPlugin}{OptimizeJavaScript};
    $text = optimizeStylesheets($text)
      if $Foswiki::cfg{PageOptimizerPlugin}{OptimizeStylesheets};
...
sub optimizeJavaScript {
    ...
    my @jsUrls = ();
    my $excludePattern =
      '(?!.*' . $Foswiki::cfg{PageOptimizerPlugin}{ExcludeJavaScript} . ')'
      if defined $Foswiki::cfg{PageOptimizerPlugin}{ExcludeJavaScript};

    while ( $text =~
        s/<script .*?src=["'](\/$excludePattern[^"']+)["'].*><\/script>/\0js\0/
      )
    {
        push @jsUrls, $1;
    }
...
sub optimizeStylesheets {
    ...
    $text =~
s/(?:<(link)\s+rel=["']stylesheet["']\s+href=["'](\/[^"']+)["']\s+media=["']all["'][^\/>]*?\/?>)|(?:<(style)\s+.*?media=["']all["'][^>]*?>(.*?)<\/style>)/_gatherCssUrls(\@cssUrls, $1||$3, $2||$4)/ges;
...
sub parseStylesheet {
    ...
    if ( -f $fileName ) {
        ...
        $data =~
          s/url\(["']?(.*?)["']?\)/"url(".rewriteUrl($1, $baseUrl).")"/ge;
...
sub purgeCache {
    ...
    my @files =
      map  { Foswiki::Sandbox::normalizeFileName( $cacheDir . '/' . $_ ) }
      grep { !/^(\.|README)/ } readdir $dh;
    closedir $dh;
